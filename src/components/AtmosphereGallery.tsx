...
export default function AtmosphereGallery() {
  return (
    <Section
      id="moments"
      className="bg-gradient-to-b from-[#0d0c0b] via-[#181411] to-[#0a0908] relative overflow-hidden"
    > 
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,180,120,0.06),transparent_65%)]" />
      <div className="space-y-12">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          ...
          <p className="max-w-2xl text-lg leading-8 text-zinc-300 lg:justify-self-end">
            The atmosphere is built from small details — the entrance, the light, the plants, the music, and the slower pace that makes Cosmic Temple feel calm and welcoming in Rawai, Phuket.
          </p>
        </div>
        ...
        <div className="grid auto-rows-[minmax(260px,auto)] gap-5 lg:grid-cols-4">
          {moments.map((moment) => (
            <article
              key={moment.title}
              className={`group relative overflow-hidden rounded-3xl border border-white/10 bg-[#161311] ${moment.className}`}
            >
              <Image
                src={moment.image}
                alt={moment.title}
                ...
                className="object-cover transition duration-[1600ms] group-hover:scale-[1.03]"
              />

              <div className="absolute inset-0 bg-gradient-to-t from-[#0d0c0b]/90 via-black/25 to-transparent" />
